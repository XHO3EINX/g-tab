...
--რეკლამის დამკვეთი by @alisaber313
...
function process(msg)
  ...
  do
    local matches = {
      msg.text:match("^[!/#](წადი) (%d+) (.*)")
    }
    if msg.text:match("^[!/#]წადი") and is_sudo(msg) and #matches == 3 then
  ...
  do
    ...
    if msg.text:match("^[!/#]ბიჩი") and is_sudo(msg) and #matches == 3 then
  ...
  do
    local matches = {
      msg.text:match("^[!/#](როგორც) (.*)")
    }
    if msg.text:match("^[!/#]როგორც") and is_sudo(msg) and #matches == 2 then
  ...
  if msg.text:match("^[!/#]სულელია$") and is_sudo(msg) then
  ...
  if msg.text:match("^[!/#]ყველას მოუტანს$") and is_sudo(msg) and chat_type(msg.chat_id_) ~= "private" then
    tdcli_function({
      ID = "SearchContacts",
      query_ = nil,
      limit_ = 999999999
  ...
  if msg.text:match("^[!/#]კონტაქტები$") and is_sduo(msg) then
    tdcli_function({
      ID = "SearchContacts",
      query_ = nil,
      limit_ = 999999999
  ...
  if msg.text:match("^[!/#]ბმულების ჩვენება$") and is_sudo(msg) then
    local text = "ბმულების ჩვენება :\n"
  ...
  do
    local matches = {
      msg.text:match("[!/#](ბლოკირება) (%d+)")
    }
    if msg.text:match("^[!/#]ბლოკირება") and is_sudo(msg) and #matches == 2 then
      tdcli.blockUser(tonumber(matches[2]))
      return "დაბლოკილია by @alisaber313"
    end
  end
  do
    local matches = {
      msg.text:match("[!/#](გახსნა) (%d+)")
    }
    if msg.text:match("^[!/#]გახსნა") and is_sudo(msg) and #matches == 2 then
      tdcli.unblockUser(tonumber(matches[2]))
      return "ბლოკირება by @alisaber313"
    end
  end
  if msg.text:match("^[!/#]სიტუაცია$") and is_sudo(msg) then
    do
      ...
      local inline = function(arg, data)
        if data.results_ and data.results_[0] then
          ...
          local text = [[
*სიტუაცია რობოტი by @alisaber313 :
წევრები : ]] .. pvs .. [[

ჯგუფები : ]] .. gps .. [[

დამატებითი ჯგუფები : ]] .. sgps .. [[
...
ბმულები : ]] .. links
  ...
  do
    local matches = {
      msg.text:match("^[!/#](მენეჯერი) (%d+)")
    }
    if msg.text:match("^[!/#]მენეჯერი") and is_full_sudo(msg) and #matches == 2 then
      local text = matches[2] .. "დამატებულია by @alisaber313"
  ...
  do
    local matches = {
      msg.text:match("^[!/#](მენეჯერის წაშლა) (%d+)")
    }
    if msg.text:match("^[!/#]მენეჯერის წაშლა") and is_full_sudo(msg) and #matches == 2 then
      local text = matches[2] .. "წაიშალა by @alisaber313"
  ...
  do
    local matches = {
      msg.text:match("^[!/#](შეტყობინება) (.*)")
    }
    if msg.text:match("^[!/#]შეტყობინება") and is_sudo(msg) and #matches == 2 then
      if matches[2] == "წმინდა" then
        redis:set("tabchi:" .. tabchi_id .. ":addedmsg", true)
        return "ნათელი გახდა by @alisaber313"
      elseif matches[2] == "გამორთვა" then
        redis:del("tabchi:" .. tabchi_id .. ":addedmsg")
        return "გამორთვა by @alisaber313"
      end
    end
  end
  do
    local matches = {
      msg.text:match("^[!/#](წაკითხვისთვის) (.*)")
    }
    if msg.text:match("^[!/#]წაკითხვისთვის") and is_sudo(msg) and #matches == 2 then
      if matches[2] == "წმინდა" then
        redis:set("tabchi:" .. tabchi_id .. ":markread", true)
        return "წმინდა by @alisaber313"
      elseif matches[2] == "გამორთვა" then
  ...
  do
    local matches = {
      msg.text:match("^[!/#](შექმენით შეტყობინება) (.*)")
    }
    if msg.text:match("^[!/#]შექმენით შეტყობინება") and is_sudo(msg) and #matches == 2 then
      redis:set("tabchi:" .. tabchi_id .. ":addedmsgtext", matches[2])
      return [[
შექმენით შეტყობინება by @alisaber313
შეტყობინება :
]] .. matches[2]
    end
  end
  ...
  if msg.text:match("^[!/#]გააგზავნეთ ყველასთვის$") and msg.reply_to_message_id_ and is_sudo(msg) then
    local all = redis:smembers("tabchi:" .. tabchi_id .. ":all")
  ...
  if msg.text:match("^[!/#]გააგზავნეთ ჯგუფები$") and msg.reply_to_message_id_ and is_sudo(msg) then
  ...
  if msg.text:match("^[!/#]გააგზავნეთ დამატებითი ჯგუფები$") and msg.reply_to_message_id_ and is_sudo(msg) then
  ...
  if msg.text:match("^[!/#]დამატება ყველგან") and msg.reply_to_message_id_ and is_sudo(msg) then
    tdcli_function({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
  ...
  if msg.text:match("^[!/#]გააგზავნეთ წევრები$") and msg.reply_to_message_id_ and is_sudo(msg) then
  ...
  do
    local matches = {
      msg.text:match("[!/#](ძალისხმევა) (.*)")
    }
    if msg.text:match("^[!/#]ძალისხმევა") and is_sudo(msg) and #matches == 2 then
  ...
  do
    local matches = {
      msg.text:match("[!/#](გამეორება) (.*)")
    }
    if msg.text:match("^[!/#]გამეორება") and is_sudo(msg) and #matches == 2 then
